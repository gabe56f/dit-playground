...
def decode_beatmap(
    metadata: Metadata,
    labels: Float[np.ndarray, str(f"{NUM_LABELS}")],
    enc: EncodedBeatmap,
    ...
    for hit in hits:
        ...
        hos.append(
            f"{x1},{y1},{t},{2**1 + combo_bit},0,B|{curve_pts},{num_slides},{length}"
        )
        last_end_time = u
    ...
    base_slider_vel = (
        1 if len(slider_vels) == 0 else (min(slider_vels) * max(slider_vels)) ** 0.5
    )
    beat_len = (
        100 / base_slider_vel
    )  # set `slider_mult` to 1 (.4 <= `slider_mult` <= 3.6)
    ...
    for t, vel in zip(slider_ts, slider_vels):
        ...
        if SV > 10 or SV < 0.1:
            print(
                "warning: SV > 10 or SV < .1 not supported, might result in bad sliders:",
                SV,
            )
