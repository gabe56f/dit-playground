...
class Beatmap:
    ...
    def __init__(self, filename, meta_only=False):
        ...
        self.audio_filename = (
            self.filename.parent / lc_files[cfg["General"]["AudioFilename"].lower()]
        )
    ...
    def parse_timing_points(self, lines):
        ...
        for line in lines:
            ...
            if cur_beat_length is None or cur_meter is None:
                raise ValueError(
                    "inherited timing point appears before any uninherited timing points"
                )
    ...
    def parse_hit_objects(self, lines):
        ...
        for line in lines:
            ...
            x, y, t, k = [int(x) for x in split[:4]]
            new_combo = (k & (1 << 2)) > 0
            if k & (1 << 0):  # hit circle
                ho = Circle(t, new_combo, x, y)
            elif k & (1 << 1):  # slider
                curve, slides, length = split[5:8]
                _, *control_points = curve.split("|")
                control_points = [np.array([x, y], dtype=float)] + [
                    np.array(list(map(int, p.split(":"))), dtype=float)
                ...
                ho = Spinner(t, new_combo, int(split[5]))
            else:
