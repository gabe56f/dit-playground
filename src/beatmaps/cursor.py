...
from .hit_objects import Circle, Slider, Spinner, HitObject
...
def cursor_signal(bm: Beatmap, frame_times: FrameTimes) -> CursorSignal:
    ...
    for cur, nxt in zip(hos, hos[1:] + [None]):
        ...
        wait_count = np.sum(
            (frame_times >= cur.end_time()) & (frame_times < nxt.t - preempt)
        )
        ...
        pos.extend(
            (1 - approach_f[:, None]) * cur.end_pos()
            + approach_f[:, None] * nxt.start_pos()
        )
