    ...
    if len(ctrl_pts) == 3:  # check P type
        ...
        if ABC == 0:  # collinear
            if np.dot(B - A, C - B) > 0:  # A -- B -- C
                ...
                return Bezier(
                    t, beat_length, slider_mult, new_combo, slides, length, ctrl_pts
                )
...
class Bezier(Slider):
    ...
    def curve_reparameterize(
        self, t: Float[ndarray, "L"]
    ) -> tuple[Int[ndarray, "L"], Float[ndarray, "L"]]:
        """
    ...
    def vel(self, t: Float[ndarray, "L"]) -> Float[ndarray, "L 2"]:
        return np.stack(
            [
                self.path_segments[idx].evaluate_hodograph(t)[:, 0]
                / self.slide_duration
                for idx, t in zip(*self.curve_reparameterize(t))
            ],
            axis=0,
